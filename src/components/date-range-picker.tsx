...
export function DatePickerWithRange({
  ...
  const getButtonLabel = () => {
    ...
    if (isSameDay(from, subDays(today, 1)) && isSameDay(to, subDays(today, 1)))
      return "Ontem";

    if (isSameDay(from, subDays(today, 7)) && isSameDay(to, today))
      return "Últimos 7 dias";

    if (isSameDay(from, subDays(today, 30)) && isSameDay(to, today))
    ...
    if (
      isSameDay(from, startOfMonth(subMonths(today, 1))) &&
      isSameDay(to, endOfMonth(subMonths(today, 1)))
    )
      return "Mês Passado";
